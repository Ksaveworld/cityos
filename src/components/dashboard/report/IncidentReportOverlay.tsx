...
import { ArrowLeft, CheckCircle2, Download, Printer, ShieldAlert, X } from 'lucide-react'

import { resolveAssignments } from '../workflow/ApprovedOutputs'
import type { DataLabel, DomainFixture, WorkflowSession } from '../workflow/types'
...
export default function IncidentReportOverlay({
  ...
  return (
    <div ref={dialogRef} className="report-overlay fixed inset-0 z-[90] overflow-y-auto bg-[#EEF2F7]" role="dialog" aria-modal="true" aria-label={reportTitle}>
      <div className="no-print sticky top-0 z-10 flex h-12 items-center justify-between border-b border-[#DDE4EE] bg-white/95 px-4 shadow-sm backdrop-blur">
        <button type="button" onClick={onClose} className="inline-flex h-8 items-center gap-1.5 rounded-lg px-2 text-label font-semibold text-ink-2 hover:bg-page"><ArrowLeft size={14} />返回事件详情</button>
        <div className="flex items-center gap-2">
          <MetaBadge tone="blue">即时生成 · {reportSourceLabel}</MetaBadge>
          <MetaBadge tone={approved ? 'green' : 'amber'}>{approved ? '当前版本已人工批准' : '待人工批准'}</MetaBadge>
          <button type="button" onClick={() => window.print()} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-line bg-white px-3 text-label font-semibold text-ink-1 hover:bg-page"><Printer size={13} />打印 / 导出 PDF</button>
          <button type="button" onClick={onClose} aria-label="关闭报告" className="grid size-8 place-items-center rounded-lg border border-line bg-white text-ink-2 hover:bg-page"><X size={14} /></button>
        </div>
      </div>

      <main id="cityos-report" data-report-kind={view} data-report-number={reportNumber} data-report-version={session.planVersion} data-version-updated-at={versionUpdatedAt} data-execution-plan={currentExecutionLabel} data-delivery-status={deliveryLabel(session.deliveryStatus)} className="report-document mx-auto my-5 w-[1120px] rounded-[14px] bg-white px-10 py-8 shadow-[0_16px_50px_rgb(35_48_73_/_0.14)]">
        <header className="report-header">
          <div className="report-kicker">CITY OS · 城市应急协同</div>
          <h1 ref={headingRef} tabIndex={-1}>{reportTitle}</h1>
          <p>{fixture.title}</p>
        ...
        <div className="report-alert">
          ...
          <div>
            ...
            <p>{view === 'brief' ? situationSummary : view === 'plan' ? `${selectedPlan.summary}${currentPlan ? `当前数值来自本次会话 v${session.planVersion} 重算，不沿用候选模板旧值。` : '当前数值为该候选的初始估算，未按本次会话重算。'}` : isCommandTask ? `以已批准方案为执行依据，汇总事件事实、人员资源、路线时限、部门任务和回传要求。` : isOwnerTask ? `${departments.length} 项负责人任务由人工批准版本生成；批准状态与下发状态分别记录。` : `本报告记录方案 ${selectedPlan.label} 点击执行后的任务状态、实际投入、回传结果与办结状态，不再复用候选方案内容。`}</p>
          </div>
          {view === 'brief' ? (
        ...
        <footer className="report-signoff">
          ...
          <div className="no-print"><button type="button" onClick={() => window.print()}><Download size={14} />导出当前报告</button></div>
